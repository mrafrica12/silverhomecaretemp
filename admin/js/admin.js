...
async function loadLeads() {
  ...
  try {
    ...
    const res  = await fetch(url + sep + 'action=leads');
    ...
    allLeads = json.leads.slice(1); // first row is header
...
function getAgeBadge(timestamp, status) {
  if (['Contacted','Consultation Scheduled','In Progress','Converted','Not a Fit','Closed'].includes(status)) return '';
  if (!timestamp) return '';
...
function renderLeads(leads) {
  ...
  // Sort: starred leads first, then by original order
  const sorted = [...leads].sort((a, b) => {
    const aP = a[15] === true || a[15] === 'TRUE' || a[15] === 'true';
    const bP = b[15] === true || b[15] === 'TRUE' || b[15] === 'true';
  ...
  sorted.forEach((row) => {
    ...
    const [timestamp, name, , phone, email, recipient, , careNeeds, , , contactTime, , , status, , priority] = row;
    const currentStatus   = status || 'New';
    const isPriority      = priority === true || priority === 'TRUE' || priority === 'true';
    ...
    if (isPriority) tr.style.background = 'rgba(196,163,90,0.05)';
    tr.innerHTML = `
      <td onclick="event.stopPropagation()" style="text-align:center;padding:0 0.25rem;">
        <button class="priority-star ${isPriority ? 'priority-star--on' : ''}" data-row="${sheetRow}"
          onclick="togglePriority(this)" aria-label="${isPriority ? 'Remove priority' : 'Mark as priority'}" title="Priority">★</button>
      </td>
      ...
      <td class="col-hide-mobile"><a href="tel:${esc(phone)}" style="color:var(--sage-dark);font-weight:500;" onclick="event.stopPropagation()">${esc(phone || '—')}</a></td>
      <td class="col-hide-mobile"><a href="mailto:${esc(email)}" style="color:var(--sage-dark);" onclick="event.stopPropagation()">${esc(email || '—')}</a></td>
      <td class="col-hide-mobile" style="font-size:var(--text-small);color:var(--gray-600);">${esc(careNeeds || '—')}</td>
      <td class="col-hide-mobile" style="font-size:var(--text-small);color:var(--gray-600);">${esc(contactTime || '—')}</td>
      <td onclick="event.stopPropagation()">
        <select class="status-select" data-row="${sheetRow}" onchange="updateStatus(this)" aria-label="Change status"
          ...
          <option value="Consultation Scheduled"  ${currentStatus==='Consultation Scheduled'  ? 'selected':''}>📅 Consultation Scheduled</option>
          <option value="In Progress"             ${currentStatus==='In Progress'             ? 'selected':''}>🔄 In Progress</option>
          <option value="Converted"               ${currentStatus==='Converted'               ? 'selected':''}>🏆 Converted</option>
          <option value="Not a Fit"               ${currentStatus==='Not a Fit'               ? 'selected':''}>🚫 Not a Fit</option>
...
function applyStatusStyle(sel, status) {
  const styles = {
    'New':                     { background: 'rgba(29,78,216,0.1)',   color: '#1D4ED8', border: 'rgba(29,78,216,0.25)'  },
    'Contacted':               { background: 'rgba(3,105,161,0.1)',   color: '#0369A1', border: 'rgba(3,105,161,0.25)'  },
    'Consultation Scheduled':  { background: 'rgba(22,101,52,0.1)',   color: '#166534', border: 'rgba(22,101,52,0.25)'  },
    'In Progress':             { background: 'rgba(146,64,14,0.1)',   color: '#92400E', border: 'rgba(146,64,14,0.25)'  },
    'Converted':               { background: 'rgba(6,95,70,0.12)',    color: '#065F46', border: 'rgba(6,95,70,0.25)'    },
    'Not a Fit':               { background: 'rgba(153,27,27,0.08)',  color: '#991B1B', border: 'rgba(153,27,27,0.2)'   },
    'Closed':                  { background: 'rgba(55,65,81,0.08)',   color: '#374151', border: 'rgba(55,65,81,0.2)'    },
  };
...
function updateStats(leads) {
  ...
  const needAction  = leads.filter(r => {
    if (r[13] && r[13] !== 'New' && r[13] !== '') return false;
    if (!r[0]) return false;
    return (now - new Date(r[0]).getTime()) > 48 * 3_600_000;
  }).length;

  const won         = leads.filter(r => r[13] === 'Converted').length;
  const lost        = leads.filter(r => r[13] === 'Not a Fit' || r[13] === 'Closed').length;
  const totalClosed = won + lost;
  const convRate    = totalClosed > 0 ? Math.round((won / totalClosed) * 100) + '%' : '—';
...
/* ── PRIORITY FLAG ───────────────────────────────────────────── */
async function togglePriority(btn) {
  const sheetRow  = btn.dataset.row;
  const isOn      = btn.classList.contains('priority-star--on');
  const newVal    = !isOn;
  const url       = localStorage.getItem(STORAGE_KEY_URL) || DEFAULT_SCRIPT_URL;

  btn.classList.toggle('priority-star--on', newVal);
  btn.setAttribute('aria-label', newVal ? 'Remove priority' : 'Mark as priority');

  const idx = parseInt(sheetRow) - 2;
  if (allLeads[idx]) allLeads[idx][15] = newVal;

  // Update row highlight
  const tr = btn.closest('tr');
  if (tr) tr.style.background = newVal ? 'rgba(196,163,90,0.05)' : '';

  try {
    await fetch(url, {
      method: 'POST',
      mode:   'no-cors',
      body:   new URLSearchParams({ action: 'update_priority', row: sheetRow, priority: String(newVal) })
    });
  } catch {
    showAdminToast('Could not update priority — check your connection.', false);
  }
}
