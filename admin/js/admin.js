...
function getAgeBadge(timestamp, status) {
  if (status === 'Contacted' || status === 'Closed' || status === 'Closed/Won' || status === 'Closed/Lost') return '';
  if (!timestamp) return '';
    ...
    tr.innerHTML = `
      ...
      <td onclick="event.stopPropagation()">
        <select class="status-select" data-row="${sheetRow}" onchange="updateStatus(this)" aria-label="Change status"
          ...
          <option value="Closed/Won"   ${currentStatus === 'Closed/Won'   ? 'selected' : ''}>🏆 Closed/Won</option>
          <option value="Closed/Lost"  ${currentStatus === 'Closed/Lost'  ? 'selected' : ''}>⚫ Closed/Lost</option>
        </select>
      </td>
    `;
...
function applyStatusStyle(sel, status) {
  const styles = {
    ...
    'Closed/Won': { background: 'rgba(52,199,89,0.12)',   color: '#1a7a35',           border: 'rgba(52,199,89,0.3)'  },
    'Closed/Lost':{ background: 'var(--gray-200)',        color: 'var(--gray-600)',   border: 'var(--gray-300)'      },
    Closed:       { background: 'var(--gray-200)',        color: 'var(--gray-600)',   border: 'var(--gray-300)'      },
  };
...
function updateStats(leads) {
  ...
  const won   = leads.filter(r => r[13] === 'Closed/Won').length;
  const lost  = leads.filter(r => r[13] === 'Closed/Lost' || r[13] === 'Closed').length;
